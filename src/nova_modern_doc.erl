...
description(#xmlElement{name = description, content = Content}) ->
    lists:foldl(fun
                    ...
                    (#xmlElement{name = fullDescription, content = Cont}, Acc) ->
                       Acc#description{full = full_description(Cont)};
                    (_T, Acc) ->
                        Acc
               end, #description{brief = ""}, Content).

full_description([]) -> [];
full_description([#xmlText{value = Text}|Tl]) ->
    [Text|full_description(Tl)];
full_description([#xmlElement{name = Name, attributes = Attrs, content = Content}|Tl]) ->
    [{Name, flatten_attrs(Attrs), full_description(Content)}|full_description(Tl)].

flatten_attrs([]) -> [];
flatten_attrs([#xmlAttribute{name = Name, value = Value}|Tl]) ->
    [{Name, Value}|flatten_attrs(Tl)].
...
type_(#xmlElement{name = abstype} = Element) ->
    #type_abstract{
       ...
       module = get_attr(module, get_child(erlangName, Element), undefined),
       href = get_attr(href, Element, undefined),
...
function(#xmlElement{name = function, content = Content} = Function) ->
    lists:foldl(fun
                    ...
                    (#xmlElement{name = typespec, content = C0}, Acc) ->
                        typespecs(C0, Acc);
...
typespecs(Es, Function) ->
    lists:foldl(fun(E, F) ->
                        typespec(E, F)
                end, Function, Es).

typespec([], F) -> F;
typespec(#xmlElement{name = type, content = Content}, F) ->
    typespecs(Content, F);
typespec(#xmlElement{name = 'fun', content = Content}, F) ->
    %% In this structure we can grab "types" and get the return type of the function
    [ReturnType] = get_types(Content),
    typespecs(Content, F#function{returns = ReturnType});
typespec(#xmlElement{name = argtypes, content = Content}, F) ->
    Types = get_types(Content),
    typespecs(Content, F#function{types = Types ++ F#function.types});
typespec(#xmlElement{name = _}, F) ->
    F.

get_types([]) -> [];
get_types([#xmlElement{name = type, content = _Content} = E|Es]) ->
    [type(E)|get_types(Es)];
get_types([_|Es]) ->
    get_types(Es).
